...
class EditProperty extends StatefulWidget {
  final int id;
  EditProperty({Key key, this.id}) : super(key: key);
...
class _EditPropertyState extends State<EditProperty> {
  ...
  void pickImage() async {
    try {
      ...
      ImagePickerApp resImage = await ApiService()
          .uploadImage(new File(img.path), auth.token, this.widget.id, auth.user.id);
      setState(() {
              ...
              return Stack(
                children: [
                  Container(
                    ...
                    child: Image.network("${apiURL}/public/assets/images/${e.path}/${e.imageName}"),
                  ),
                  Align(
